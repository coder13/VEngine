...
public abstract class Vec2<N> {
	...
	public static class i extends Vec2<Integer> {
		...
		public Integer dist(Vec2<Integer> vec) {
			return (int) VMath.sqrt((X-vec.X)*(X-vec.X) + (Y-vec.Y)*(Y-vec.Y));
		}
		...
		public long combine(){
			long x = (long)(X<0?(long)X&0x7fffffff:X),
				 y = (long)(Y<0?(long)Y&0x7fffffff:Y);
			return x | y << 32;
		}
	...
	public static class f extends Vec2<Float> {
		...
		public Float dist(Vec2<Float> vec) {
			return (float) VMath.sqrt((X-vec.X)*(X-vec.X) + (Y-vec.Y)*(Y-vec.Y));
		}
	...
	public static class d extends Vec2<Double> {
		...
		public Double dist(Vec2<Double> vec) {
			return (double) VMath.sqrt((X-vec.X)*(X-vec.X) + (Y-vec.Y)*(Y-vec.Y));
		}
