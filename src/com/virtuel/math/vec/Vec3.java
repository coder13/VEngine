...
public abstract class Vec3<N> {
	...
	public static class i extends Vec3<Integer> {
		...
		public Vec3.i cross(Vec3<Integer> vec) {
			return new Vec3.i(Y * vec.Z - Z * vec.Y, Z * vec.X - X * vec.Z, X
					* vec.Y - Y * vec.X);
		}
		...
		public long combine() {
			long x = (long) (X < 0 ? (long) X & 0x7fffff : X), y = (long) (Y < 0 ? (long) Y & 0x7fff
					: Y), z = (long) (Z < 0 ? (long) Z & 0x7fffff : Z);
			return x << 48 | z << 24 | y;
		}
	...
	public static class f extends Vec3<Float> {
		...
		public Vec3.f cross(Vec3<Float> vec) {
			return new Vec3.f(Y * vec.Z - Z * vec.Y, Z * vec.X - X * vec.Z, X
					* vec.Y - Y * vec.X);
		}
	...
	public static class d extends Vec3<Double> {
		...
		public Vec3.d cross(Vec3<Double> vec) {
			return new Vec3.d(Y * vec.Z - Z * vec.Y, Z * vec.X - X * vec.Z, X
					* vec.Y - Y * vec.X);
		}
	...
	/** Multiplies both X, Y and Z by a. */
	...
	public static Vec3.i floor(Vec3.d vec) {
		return new Vec3.i((int) VMath.floor(vec.X), (int) VMath.floor(vec.Y),
				(int) VMath.floor(vec.Z));
	}
